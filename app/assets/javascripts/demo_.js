...
//Secret: 232b97ca4aad158d
   ...
   $.ajax({
        ...
        data: {
            ...
            //method: 'flickr.interestingness.getPhotos',
            photoset_id: '72157646534862923',
            api_key: '46435ef862fc267e21d9ae84fc20d892' // jshint ignore:line
       ...
       $.each(result.photos.photo, function (index, photo) {
           ...
           console.log("Resultado: " +photo.server);
            baseUrl = 'http://farm' + photo.farm + '.static.flickr.com/' +photo.server + '/' + photo.id + '_' + photo.secret;
            console.log(baseUrl);

            //alert("photo id: "+photo.id + "photo secret: "+photo.secret + "Photo Server: "+ photo.server);
            //alert('Base Url: '+baseUrl);
            $('<a/>')
                .append($('<img>').prop('src', baseUrl))
                .prop('href', baseUrl + '_s.jpg')
                .prop('title', photo.title)
                .attr('data-gallery', '')
                .appendTo(linksContainer);
            carouselLinks.push({
